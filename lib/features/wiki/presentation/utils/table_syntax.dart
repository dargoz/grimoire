...
import 'package:grimoire/features/wiki/presentation/utils/flavor_patterns.dart';
...
class TableSyntax extends BlockSyntax {
  ...
  Element _parseRow(
    ...
    for (final cell in cells) {
      if (hexColorPattern.hasMatch(cell)) {
        var code = Element.text('hex-color', cell);
        row.add(Element(cellType, [code]));
      } else {
        row.add(Element(cellType, [UnparsedContent(cell)]));
      }
    }
